...
async def crear_unidadmedida(unidadmMedidaParam: UnidadMedidaCreate, db: Session = Depends(get_db)):
...
async def obtener_unidadmedida(db: Session = Depends(get_db)):
...
async def actualizar_unidadmedida(id: int, unidadmMedidaParam: UnidadMedidaCreate, db: Session = Depends(get_db)):
    unidadmedida = db.query(UnidadMedida).filter(UnidadMedida.IdUnidadMedida == id).first()
    if unidadmedida is None:
        raise HTTPException(status_code=404, detail="No existe esa unidad de medida")
...
async def eliminar_unidadmedida(id: int, db: Session = Depends(get_db)):
    unidadmedida = db.query(UnidadMedida).filter(UnidadMedida.IdUnidadMedida == id).first()
    if unidadmedida is None:
        raise HTTPException(status_code=404, detail="No existe esa unidad de medida")
